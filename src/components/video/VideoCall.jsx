...
const getDeviceType = () => {
    ...
    if (
        screenWidth <= 767 ||
        /Android|iPhone|iPod|BlackBerry|IEMobile|Opera Mini|webOS/i.test(
            userAgent
        )
    ) {
        return "mobile";
...
const MobileFloatingParticipant = React.memo(
    ({ participant, isCameraOff, call }) => {
        ...
        const videoTrackId =
            participant?.videoStream?.getVideoTracks()?.[0]?.id;
        ...
        useEffect(() => {
            ...
            if (video && !isCameraOff && currentVideoTrack) {
                const debouncedUpdateAspectRatio = () => {
                    ...
                    aspectRatioTimeoutRef.current = setTimeout(
                        updateAspectRatio,
                        150
                    );
                };
...
const TabletFloatingParticipant = React.memo(
    ({ participant, isCameraOff, call }) => {
        const videoRef = useRef(null);
        const containerRef = useRef(null);
        const [dynamicAspectRatio, setDynamicAspectRatio] = useState("9/16"); // fallback to phone camera ratio
...
const DesktopFloatingParticipant = ({ isCameraOff }) => {
    ...
    const { aspectRatio, attachVideoElement } = useVideoAspectRatio(
        !isCameraOff
    );
    ...
    useEffect(() => {
        if (floatingContainerRef.current) {
            if (!isCameraOff) {
                ...
                floatingContainerRef.current.style.removeProperty(
                    "--local-video-aspect-ratio"
                );
    ...
    useEffect(() => {
        if (!isCameraOff && floatingContainerRef.current) {
            ...
            const findVideoElement = () => {
                const videoElement =
                    floatingContainerRef.current.querySelector("video");
                if (videoElement) {
    ...
    return (
        <div className="floating-local-participant" ref={floatingContainerRef}>
...
const FloatingLocalParticipant = ({ isCameraOff, call }) => {
    ...
    switch (deviceType) {
        ...
        case "desktop":
        default:
            return <DesktopFloatingParticipant isCameraOff={isCameraOff} />;
    }
};
...
const CustomControlButton = ({
    ...
    <button
        onClick={onClick}
        disabled={disabled}
        className={`custom-control-btn ${className} ${
            isActive ? "active" : ""
        }`}
    >
        {children}
...
const VideoCallUI = ({
    ...
    // Auto-close functionality - Simplified and more reliable
            ...
            if (call) {
                if (isMicMuted) {
                    ...
                    if (
                        ...
                        call.audio &&
                        typeof call.audio.enable === "function"
                    ) {
                    ...
                    if (
                        ...
                        call.audio &&
                        typeof call.audio.disable === "function"
                    ) {
            ...
            if (call) {
                if (isCameraOff) {
                    ...
                    if (
                        ...
                        call.video &&
                        typeof call.video.enable === "function"
                    ) {
                    ...
                    if (
                        ...
                        call.video &&
                        typeof call.video.disable === "function"
                    ) {
    ...
    // Check browser capability for auto-close on component mount
    useEffect(() => {
        const checkAutoCloseCapability = () => {
            // Check if we can potentially close the tab
            const canClose = !!(window.opener || window.history.length === 1);
            setCanAutoClose(canClose);
            console.log("Auto-close capability:", canClose);
        };

        checkAutoCloseCapability();
    }, []);

    // Simplified and more reliable countdown function
    const startCountdown = (seconds, type) => {
        if (hasStartedCountdownRef.current) return;

        console.log(`Starting ${type} countdown: ${seconds} seconds`);
        ...
        countdownIntervalRef.current = setInterval(() => {
            currentCount -= 1;
            console.log(`${type} countdown tick:`, currentCount);
            ...
            if (currentCount <= 0) {
                ...
                console.log(`${type} countdown finished, attempting to close`);

                if (type === "manual") {
                    onLeave(); // Call onLeave for manual close
                }

                // Enhanced close attempt with better browser detection
                const attemptClose = () => {
                    try {
                        // Method 1: Check if opened by script
                        if (window.opener) {
                            window.close();
                            return true;
                        }

                        // Method 2: Check if single page in history
                        ...
                        console.error("Close attempt failed:", error);
                        return false;
                    }
                };
                ...
                if (!closed) {
                    // Show enhanced fallback UI instead of alert
                    setTimeout(() => {
                        console.log("Auto-close failed, showing fallback UI");
                        setIsAutoClosing(false);
                        setShowCloseMessage(true); // Keep showing message but with manual options
    ...
    useEffect(() => {
        console.log("Auto-close effect triggered:", {
            callEndedAt,
            hasStarted: hasStartedCountdownRef.current,
        });

        if (callEndedAt && !hasStartedCountdownRef.current) {
            startCountdown(5, "auto");
        }
                ...
                <div className="call-ended-content">
                    ...
                    {isAutoClosing && (
                        <div className="auto-close-countdown">
                            ...
                            <p>
                                Tab will close automatically in {countdown}{" "}
    ...
    return (
        <StreamTheme>
            <div className="video-call-ui">
                ...
                <div
                    className={`custom-call-controls ${
...
const PreCallSetup = ({
    ...
    const { aspectRatio, naturalSize } = usePreviewAspectRatio(
        videoRef.current,
        isCameraEnabled
    );
