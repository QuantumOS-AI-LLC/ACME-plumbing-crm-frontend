...
const EstimateCard = ({ estimate, onClick, onViewClick, onUpdate }) => {
  ...
  const transformedEstimate = {
    ...estimate,
    name: estimate?.leadName || estimate?.client?.name || "Untitled Estimate",
  ...
  const handleViewClick = (e) => {
    ...
    if (onViewClick) {
      ...
      console.warn("onViewClick prop is not provided!");
    }
  };
    ...
    try {
      ...
      if (newStatus === ESTIMATE_STATUS.ACCEPTED) {
        const acceptedNotification = {
          id: `estimate-accepted-${transformedEstimate.id}-${Date.now()}`,
      ...
      // Update the estimate with the new status - this will trigger tab refresh and navigation if needed
      onUpdate(result.data);
      // Note: Toast notification is now handled in EstimatesPage for better UX with tab navigation
    } catch (error) {
        ...
        <CardContent sx={{ padding: 3, paddingTop: 2, paddingBottom: 2 }}>
          ...
          <Grid container spacing={2} sx={{ mb: 2 }}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <Typography variant="body1" sx={{ color: "text.primary", pl: 3 }}>
          ...
          >
            ...
            >
              <Select
                ...
                sx={{
                  borderRadius: 2,
                  backgroundColor: loading ? "grey.50" : "background.default",
