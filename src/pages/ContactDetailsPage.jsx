...
import ConversationHistory from "../components/contacts/ConversationHistory";
...
const ContactDetailsPage = () => {
  ...
  const loadAllJobCounts = useCallback(async () => {
    try {
      ...
      if (!apiWorked) {
        ...
        counts.open = fakeJobs.filter((job) => job.status === "open").length;
  ...
  const loadJobsForContact = useCallback(
    async (status = null) => {
      try {
        ...
        if (status && status !== "all") {
          filteredFakeJobs = fakeJobs.filter((job) => job.status === status);
        }
  ...
  const handleSaveContact = async () => {
    try {
      ...
      if (response && response.data) {
        console.log("ContactDetailsPage: Updated contact", response.data);
        setContact(response.data);
        setOpenEditDialog(false);
  ...
  return (
    <Box>
        ...
        {jobsLoading ? (
          ...
          <Box sx={{ textAlign: "center", py: 4 }}>
            <Typography variant="body1" color="text.secondary">
              No {activeJobTab.replace("_", " ")} jobs found for {contact.name}.
            </Typography>
          </Box>
        ) : (
      ...
      <Paper sx={{ p: 3 }}>
        <ConversationHistory contactId={id} contactName={contact.name} />
      </Paper>
            ...
            <Grid item xs={12}>
              ...
              >
                ...
                <label htmlFor="enableRecording">
                  <Typography variant="body1">Enable Recording</Typography>
                </label>
              </Box>
