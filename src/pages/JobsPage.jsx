...
const JobsPage = () => {
  ...
  const { jobs, loading, error, loadJobs, updateJobInState, addJobToState } =
    useJobs();
  ...
  const handleJobUpdate = (updatedJob) => {
    // Update the jobs state using context
  ...
  const handleStatusChange = async (jobId, newStatus) => {
    try {
      ...
      // Update the jobs state using context
      updateJobInState({ id: jobId, status: newStatus });
  ...
  const handleJobCreated = (newJob) => {
    // Add the new job to context state
    if (newJob) {
      addJobToState(newJob);
    } else {
      // Fallback: refresh jobs list if no job data provided
      loadJobs();
    }
  };
